...
function ActionMenu({ poId, openMenu, setOpenMenu }) {
  const isOpen = openMenu === poId;
  ...
  function handleToggle() {
    ...
    setOpenMenu(isOpen ? null : poId);
  }

  function handleAction(id) {
    setOpenMenu(null);
    if (id === 'view') window.location.assign(`#/billing/purchase-order/${poId}/view`);
    else if (id === 'edit') window.location.assign(`#/billing/purchase-order/${poId}/edit`);
  }
...
export function PurchaseOrderPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setError('');
      try {
        const response = await api.listInvoices({ documentType: 'purchase-order', limit: 100 });
        const data = Array.isArray(response.data) ? response.data.map(normalizePO) : [];
        setOrders(data);
      } catch (err) {
      ...
      <div className="bg-white border border-[#dfe7f1] rounded-lg overflow-hidden">
          ...
          <table className="w-full border-collapse" style={{ minWidth: 1020 }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((o) => (
                  <tr key={o.id} className="border-t border-[#edf2f7] hover:bg-[#fafbfe] transition-colors">
                    ...
                    {/* Actions */}
                    <td className="px-4 py-3.5 text-right">
                      <ActionMenu poId={o.id} openMenu={openMenu} setOpenMenu={setOpenMenu} />
                    </td>
                  </tr>
                ))
              )}
        ...
        <div className="px-4 py-3 border-t border-[#edf2f7] flex flex-wrap items-center justify-between gap-2">
          <span className="text-[13px] text-[#536173]">
            ...
            <span className="font-medium text-[#374151]">{orders.length}</span> purchase orders
          </span>
